...
*/
ADS130B04::ADS130B04(uint8_t _chipSelectPin,
...
*/
int8_t ADS130B04::begin(uint16_t _fs) {

  int8_t result = ADS130B04_OK;
...
*/
int8_t ADS130B04::setSPS(uint16_t sps) 
{
...
*/
int8_t ADS130B04::setResolution(uint8_t res) 
{
  if (res < 5) {
      pwr = res;
      return writeClockRegister();
...
*/
int8_t ADS130B04::enableChannels(bool ch0, bool ch1, bool ch2, bool ch3) 
{
...
*/
int8_t ADS130B04::enableChannels(byte channelMask) 
{
  enabledChannels = channelMask & 0x0F;
  writeClockRegister();
}
...
*/
int8_t ADS130B04::writeClockRegister() 
{
  ...
  // 0b0000110110011110
  if (writeRegister(ADS130B04_ADDR_CLOCK, clockRegData) == 0)
...
/*int ADS130B04::setClock(bool internalClock) {
...
*/
int8_t ADS130B04::status() {
  //Serial.println("Checking ADS130B04 status");
...
/*! This assumes that the previous command was either a status() or
    ...
    enabled.
*/
int8_t ADS130B04::read(int16_t* data) {
...
*/
int8_t ADS130B04::reset() {
  Serial.println("Resetting ADS130B04");
  ...
  int result = ADS130B04_OK;
  
  if (b1 != 0xFF) {
    result = ADS130B04_ERROR;
    Serial.print("Umm ");
...
/*! Set to standby. 
*/
int8_t ADS130B04::standby() {
  //Serial.println("Setting ADS130B04 to standby");
...
/*! Lock the device.
*/
int8_t ADS130B04::lock() {
  Serial.println("Locking ADS130B04");

  return 0;
}
  
//! Unlock device
int8_t ADS130B04::unlock() {
  Serial.println("Unlocking ADS130B04");

  return 0;
}

// Read from register. THIS WILL WORK WITH UP TO 2 BYTES.
int8_t ADS130B04::readRegister(byte thisRegister, uint8_t bytesToRead) {

  byte inByte = 0;           // incoming byte from the SPI

  unsigned int result = ADS130B04_OK;   // result to return
...
*/
int8_t ADS130B04::writeRegister(byte thisRegister, uint16_t thisValue) {
  ...
  if ((b1 >> 5) != 0b010) {
    result = ADS130B04_ERROR_WRITING_TO_REGISTER;
  }
  else {
    byte regAddr =  ((b1 << 1) | (b2 >> 7)) & 0b00111111;
    if (regAddr != thisRegister)
      result = ADS130B04_INCORRECT_REGISTER;
  }
